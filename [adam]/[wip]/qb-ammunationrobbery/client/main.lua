...
Citizen.CreateThread(function()
    while true do 
        ...
        if LocalPlayer.state.isLoggedIn then
			...
			if CurrentCops >= Config.MinimumAmmuRobberyPolice then
...
function GrabItem(spot)
    ...
    QBCore.Functions.Progressbar("grab_ifruititem", "Removing Items..", 20000, false, true, {
        ...
        --     --     TriggerServerEvent("qb-ammunationrobbery:server:callCops", streetLabel, pos)
        --     --     copsCalled = true
        --     -- end
        -- end
        ...
        TriggerServerEvent("qb-ammunationrobbery:server:PoliceAlertMessage")
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), "anim@gangops@facility@servers@", "hotwire", 1.0)
        TriggerServerEvent('qb-ammunationrobbery:server:setSpotState', "isBusy", false, spot)
    ...
    if usingSafe then
        if won then
            if not Config.Locations["safe"].isDone then
                ...
                TriggerServerEvent("qb-ammunationrobbery:server:SafeReward")
                TriggerServerEvent("qb-ammunationrobbery:server:SetSafeStatus", "isBusy", false)
                TriggerServerEvent("qb-ammunationrobbery:server:SetSafeStatus", "isDone", false)
                takeAnim()
            end
        end
    end
...
RegisterNetEvent('qb-ammunationrobbery:client:LoadList')
AddEventHandler('qb-ammunationrobbery:client:LoadList', function(list)
    ...
    if #(pos - vector3(Config.Locations["thermite"].x, Config.Locations["thermite"].y,Config.Locations["thermite"].z)) < 1.0 then
        if CurrentCops >= 0 then
            ...
            if requiredItemsShowed then
                ...
                requiredItemsShowed = false
                TriggerEvent('inventory:client:requiredItems', requiredItems, false)
                TriggerServerEvent("qb-ammunationrobbery:server:SetThermiteStatus", "isBusy", true)
...
RegisterNetEvent('qb-ammunationrobbery:client:setSpotState')
AddEventHandler('qb-ammunationrobbery:client:setSpotState', function(stateType, state, spot)
...
RegisterNetEvent('qb-ammunationrobbery:client:SetSafeStatus')
AddEventHandler('qb-ammunationrobbery:client:SetSafeStatus', function(stateType, state)
    if stateType == "isBusy" then
...
RegisterNetEvent('qb-ammunationrobbery:client:SetThermiteStatus')
AddEventHandler('qb-ammunationrobbery:client:SetThermiteStatus', function(stateType, state)
    if stateType == "isBusy" then
...
-- RegisterNetEvent('qb-ammunationrobbery:client:PoliceAlertMessage')
-- AddEventHandler('qb-ammunationrobbery:client:PoliceAlertMessage', function(msg, coords, blip)
--     if blip then
...
RegisterNUICallback('thermitefailed', function()
    -- TriggerServerEvent('qb-ammunationrobbery:server:PoliceAlertMessage2')
...
-- RegisterNetEvent('qb-ammunationrobbery:client:robberyCall')
-- AddEventHandler('qb-ammunationrobbery:client:robberyCall', function(streetLabel, coords)
